...
- [[Refresher Date Override - SA-Only Absolute-Date Override]] — **design/decision** (2026-07-19, grill): instead of making the 2y period configurable, add **one** SA-only absolute-date override that replaces the computed mandatory-training refresher date at **two scopes** — per-instructor slot and a course-wide fleet stamp (last-write-wins; a real completion wipes the override). 7 mandatory courses only. FE next session (tat-prereq + tat-ws); backend handed to BE. Ticket TBD → [[Dania]]
