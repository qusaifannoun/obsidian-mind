...
- [[TAT-409 Staff Management Subsystem]] — **new** internal subsystem (instructors + TORs) on an SSO subdomain; 21-ticket epic. Domain: [[Staff Management Subsystem & TOR Model]]. **History Form slice (417/418/419/420/421/429) wired to the real backend + verified across roles, rendered as the single-document TAT Form 031 (2026-06-28).**
  ...
  - [[tat-prereq Forms Refactor - Zod + RHF]] — **debt, raised by Qusai (2026-07-12)**: `AssessmentFormView` (22 `useState`, no schema) and `Form32Editor` (11) bypass the repo's mandatory `useZodForm` + `RHFInput` pattern and hand-roll field components that already exist. Root cause: the convention lives only in a hook docstring — **there is no `CLAUDE.md` in [[tat-prereq]]**
