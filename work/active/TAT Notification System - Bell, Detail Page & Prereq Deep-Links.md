...
- **Why this is better:** future host changes to `STAFF_MANAGEMENT_URL` (or dashboard) now just work with a redeploy — no reseed, no re-bake. See [[Gotchas - Backend Services & Environment#Notification URLs baked at seed time + skip-existing seeding = env-var changes silently ignored (2026-07-09)]].
