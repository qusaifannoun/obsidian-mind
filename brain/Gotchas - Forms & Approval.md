...
>
> **Correction (2026-07-09): the "aircraft-quals are count-only, no write API" claim above was wrong** — the backend already had `GET/POST/PATCH …/history-form/aircraft-qualifications` (`listAircraftQualifications`/`addAircraftQualification`/`replaceAircraftQualification`, owner-or-privileged), it was only the FE that stubbed the section as read-only with a "no endpoint yet" note. Built the FE add/edit table this day (list type + B1/B2 category, owner/SA/AD/QM/TM can edit; no delete endpoint). **Lesson: an FE "// no backend yet" stub comment is not evidence — grep the backend controller before trusting it.** See [[Patterns - Backend & Domain#Multi-item reject flows: accumulate-and-send only when items share ONE parent status (tat-prereq / tat-app-ws)]] neighbour and [[Staff Management Subsystem & TOR Model]].
