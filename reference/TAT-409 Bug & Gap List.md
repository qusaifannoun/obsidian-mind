...
> - **Still open (13, exact — recount 2026-07-07):** **C5** SSO initiator; **M1** training-history approval, **M2** instructor selection, **M3** assessment type, **M7** eligible-instructor already-assigned undercheck, **M11** upload categories; **L3** placeholder refresher content, **L4/L5** matrix, **L6**, **L9**, **L10**, **L11**. (Product-decision items needing BA sign-off: M1, M2, M3, L6, L9. The rest are FE/config.) Prior "~13" prose had silently dropped **M7** while wrongly listing **C2** as open — C2 is now confirmed fixed (see its row). See [[TAT-409 Ticket Groups & Inspection Map]] for per-group post-fix verdicts.
...
| C2 | 422 AC-01 | ~~**Nullable-enum crash blocks all aircraft-qual creates.**~~ **✅ FIXED** — `enum` now includes `null` so `default:null` passes validation. | `tat-app-ws` | `staff-qualification.schema.ts:123-128` now reads `enum:[...Object.values(StaffAircraftRefresherUpdateSource), null]`. Committed `db6922f7` (2026-07-05), verified via git blame 2026-07-07 — the earlier "applied-then-reverted" note was stale; fix is live on `main`. |
