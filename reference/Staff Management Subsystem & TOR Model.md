...
**TOR is the single source of truth for instructor assignment eligibility.** Existing license/role/document/course/aircraft checks become *inputs into* TOR status, not separate rules. An instructor is assignable only with a **matching-license TOR in `Active`** plus a valid requested qualification matching the required **aircraft type and/or role**. Draft/Paused instructors are **hidden entirely** from assignment lists — **no override** for any role. Applies to course/examiner/assessor/assessment/aircraft assignment, single and bulk. Related: [[TAT-429 Sit-In Eligibility & Move Semantics|TAT-429]] adds instructors to the course enrollment list (feeds the History Form Sit-In flow in TAT-421 — see [[TAT-409 Staff Management Subsystem]]).

> [!important] TAT-424 governs **qualification**, not **pre-qualification** — the sit-in path is exempt
> Applying the Active-TOR rule to [[TAT-429 Sit-In Eligibility & Move Semantics|TAT-429]]'s Add Instructor list is **circular** and was fixed on 2026-07-12 (see [[TAT-429 Sit-In Eligibility & Move Semantics]]). "Can this person *teach*?" is a TAT-424 question. "Can this person *sit in to learn*, so they can one day teach?" is not — the candidate has no TOR yet by definition. **Gate the qualification flows on TOR; never gate the bootstrap flow on it.**
